...
| 任务 | 说明 | 云文档 | md 存档 |
|------|------|--------|---------|
| 🤖 AI Builders Digest | 每日 AI 行业资讯 8 维度摘要 | ✅ | ✅ `ai-digest/YYYY-MM-DD.md` |
| 📋 每日经济政策资讯 | 10 大官媒政策整合 | ✅ | ✅ `economic-policy/YYYY-MM-DD.md` |
| 🛠️ 每日热门Skill更新 | 热门 Skills 汇总 | ✅ | ❌（仅飞书推送，不存档） |
| 📈 半导体设备股票行情 | A 股半导体设备板块播报 | ✅ | ❌（仅飞书推送，不存档） |
| 🌍 互联网出海开发专题集 | 出海开发 Top 5 文章 | ✅ | ✅ `overseas-dev/YYYY-MM-DD.md` |
| 💭 Memory Dreaming Promotion | Memory 增强任务 | ✅ | ❌（仅飞书推送，不存档） |

**说明：**
- **云文档**：任务完成后通过 openclaw 内置 delivery 发送到飞书/微信
- **md 存档**：仅限上述 3 个任务，内容统一存放在 [content-archive](https://github.com/dynamicNing/content-archive) 对应子目录，主目录不存放文件
...
| GET | `/api/cloud-docs` | 获取派群飞书云文档记录（从 cron runs 提取） |
| GET | `/api/skills` | 获取所有 Skills 列表 |
| POST | `/api/image/generate` | 生成图片（POST JSON: `{prompt, inputImage?}`） |
| GET | `/api/image/list` | 获取已生成图片列表 |
| GET | `/api/hooks` | 获取所有可用 Hooks |
| POST | `/api/hooks/:id/execute` | 触发执行指定 Hook |
| GET | `/api/hooks/:id/runs` | 获取 Hook 执行历史 |
...
| Hook ID | 名称 | 说明 |
|---------|------|------|
| `folo-ai-deploy` | 🚀 部署 folo-ai | 拉取最新代码并重启 PM2 |
| `skill-update` | 🛠️ 更新 Skills | 扫描并更新所有 Skills |
| `content-sync` | 📦 同步内容存档 | 同步内容到 content-archive |
| `skills-sync-console` | 🛠️ 同步 Skills | 同步 Skills 到 workflow-console |

```bash
# 触发 folo-ai 部署
curl -X POST https://console.folo-ai.com/api/hooks/folo-ai-deploy/execute

# 查看执行历史
curl https://console.folo-ai.com/api/hooks/folo-ai-deploy/runs
```
...
**规则：**
- 主目录 `content-archive/` 不存放任何 md 文件
- 仅以下 3 个 cron 任务允许生成 md 存档：
  - `14e827f4` (AI Builders Digest) → `ai-digest/YYYY-MM-DD.md`
  - `52542803` (每日经济政策资讯) → `economic-policy/YYYY-MM-DD.md`
  - `d92e41f3` (互联网出海开发专题集) → `overseas-dev/YYYY-MM-DD.md`
- 其他任务（Skill 更新 / 股票行情 / Memory）仅推送飞书云文档，不生成 md 存档

**仓库结构：**
```
content-archive/           ← 主目录无 md 文件
├── ai-digest/            ← 14e827f4 存档
│   └── 2026-04-16.md
├── economic-policy/      ← 52542803 存档
│   └── 2026-04-16.md
└── overseas-dev/          ← d92e41f3 存档
    └── 2026-04-16.md
```
